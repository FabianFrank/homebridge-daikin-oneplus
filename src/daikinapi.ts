...
export class DaikinApi{
    ...
    private _lastUpdateTimeNs = BigInt(-1)
    private _nextUpdateTimeNs = BigInt(-1)
    private _noUpdateBeforeNs = BigInt(0)
    ...
    async getData(){
      ...
      this._nextUpdateTimeNs = BigInt(-1);
     ...
     * 
     * ASAP: Either pull an update right now or as soon as DAIKIN_DEVICE_FOREGROUND_REFRESH_MS has
     *       passed since the last update.
     * 
     * Regular: Pull an update every 180s
     * 
     * If `blockFutureMs` is given and > 0 then no updates are guaranteed
     * to take place in the next `blockFutureMs` miliseconds.
     * @param blockUntilMs 
    ...
    _scheduleUpdate(blockUntilMs?: number, asap = false) {
      if (asap) {
        ...
        if (this._monotonic_clock_ns()-this._lastUpdateTimeNs > BigInt(DAIKIN_DEVICE_FOREGROUND_REFRESH_MS)*BigInt(1000000)) {
          this.log(LoggerLevel.ERROR, 'instant refresh now');
          this._updateIn(0);
        } else {
          const sinceLastUpdateMs = Number((this._monotonic_clock_ns() - this._lastUpdateTimeNs) / BigInt(1000000));
          const updateIn = DAIKIN_DEVICE_FOREGROUND_REFRESH_MS - sinceLastUpdateMs;
          this.log(LoggerLevel.ERROR, `instant refresh in ${updateIn}`);
          this._updateIn(updateIn);
        }
      ...
      if (!blockUntilMs) {
        ...
        nextUpdateIn = asap ? DAIKIN_DEVICE_FOREGROUND_REFRESH_MS : DAIKIN_DEVICE_BACKGROUND_REFRESH_MS;
        ...
        blockUntilMs = DAIKIN_DEVICE_FOREGROUND_REFRESH_MS;
        nextUpdateIn = DAIKIN_DEVICE_FOREGROUND_REFRESH_MS;
      } else {
        nextUpdateIn = asap ? DAIKIN_DEVICE_FOREGROUND_REFRESH_MS : DAIKIN_DEVICE_BACKGROUND_REFRESH_MS;
      }
      this._noUpdateBeforeNs = this._monotonic_clock_ns() + BigInt(blockUntilMs)*BigInt(1000000);

      const scheduledRunInNs = this._nextUpdateTimeNs - this._monotonic_clock_ns();
      if (this._nextUpdateTimeNs === BigInt(-1) || BigInt(blockUntilMs*1000000) > scheduledRunInNs) {
        // if no run is scheduled at all OR if a run is scheduled for sooner than the desired minimum wait, push it into the future
        this._updateIn(nextUpdateIn);
      } else {
        // if the next update is already far enough in the future, nothing else to do
        this.log(LoggerLevel.INFO, `not rescheduling because ${blockUntilMs} is after ${Number(scheduledRunInNs/BigInt(1000000))}`);
      }
    }
    ...
    private _updateIn(nextUpdateMs: number) {
      ...
      this._updateTimeout = setTimeout(async () => {
        await this.getData();
        this._lastUpdateTimeNs = this._monotonic_clock_ns();
      }, nextUpdateMs);
      this._nextUpdateTimeNs = this._monotonic_clock_ns() + BigInt(nextUpdateMs) * BigInt(1000000);
      this.log(LoggerLevel.INFO, `scheduled update in ${nextUpdateMs}`);
    }

    private _monotonic_clock_ns() {
